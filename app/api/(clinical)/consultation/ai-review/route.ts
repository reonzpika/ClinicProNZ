...
- Reference BPAC/HealthPathways where applicable`,
    ...
    user: (note: string) => `Review this consultation and suggest management optimisations across treatment, patient advice, and safety netting.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!problemsText && !objectiveText && !assessmentText && !planText) {
      return NextResponse.json({ error: 'No consultation content provided' }, { status: 400 });
    }
...
`.trim();
    ...
    const message = await anthropic.messages.create({
      model: process.env.ANTHROPIC_AI_REVIEW_MODEL ?? 'claude-sonnet-4-5-20250929',
      max_tokens: 2048,
      temperature: 0,
      system: promptConfig.system,
      messages: [{ role: 'user', content: promptConfig.user(fullNote) }],
    });
    ...
    db.insert(aiSuggestions)
      ...
      .catch((err) => {
        console.error('[CRITICAL] Failed to log AI suggestion:', {
          userId: context.userId,
          sessionId,
          reviewType,
    ...
    return NextResponse.json(
      { error: message, code: 'AI_REVIEW_ERROR' },
      { status: 500 },
    );
  }
}
