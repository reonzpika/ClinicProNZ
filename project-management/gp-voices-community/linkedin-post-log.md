# LinkedIn Post Log

**Purpose**: Track weekly LinkedIn posts, metrics, learnings, and all articles shared.

---

## Article Database

### Week 1: 2025-11-08

**Posted**: 2025-11-08  
**Topic**: AI in healthcare - scribe technology and AI-enabled practice management systems  
**Items**: 4 items

**Articles (Details not captured in original post)**:
- Item details not available in project files

**Metrics**:
- Profile viewers from this post: 3
- Followers gained from this post: **8 new followers**
- Reactions: 3
- Comments: 8
- Saves: 2
- Link clicks: 1

**Audience Analysis**:
- ⚠️ **Most reactions from AI tool vendors, not GPs/practice managers**
- Critical insight: engagement volume ≠ validation
- Need to track WHO is engaging, not just HOW MANY

**Notable Engagement**:
- @Octans Care replied: Platform-agnostic approach, digital front door vision
- @Terry Cornick replied: Adding companies to HTD ANZ Healthtech Directory

**Learnings**:
- Platform-agnostic messaging resonates with GPs
- Directory/resources are valuable to community
- AI tool mentions generate engagement from vendors
- Vendor engagement doesn't validate demand from target audience
- 8 new followers is promising, but need to verify they're GPs
- 2 saves suggests content has reference value
- Low link clicks (1) - may need stronger CTA

**Replies Sent**:
- Octans Care: Acknowledged platform-agnostic approach, asked GPs about digital front door vision
- Terry Cornick: Thanked for directory addition, acknowledged value to ecosystem

---

### Week 2: 2025-11-17

**Status**: Ready to Post  
**Topic**: Specialist AI Tools Showing Promise  
**Items**: 3 items included (15 items evaluated)

#### ✅ Articles Included in Post

**1. ProCare wins 2025 Cooperative Business New Zealand Innovation Award**
- **Source**: NZ Doctor
- **Type**: [NZ] News
- **URL**: https://www.nzdoctor.co.nz/article/undoctored/procare-wins-2025-cooperative-business-new-zealand-innovation-award
- **Summary**: ProCare has been named winner of the 2025 Cooperative Business New Zealand Innovation Award. The organisation is integrating AI tools, particularly an AI-powered medical scribe from Heidi Health, to ease GP workload and improve patient care. Key initiatives include: partnership with Heidi Health for AI medical scribe technology; Health Accelerator joint venture with Pegasus, Pinnacle, and Tū Ora Compass Health to fast-track AI solutions across 500+ practices; innovation award recognition; and "Using AI in Primary Care" online course for professional development.
- **GP Relevance**: HIGH - Primary care innovation, AI integration in NZ general practice
- **Engagement Potential**: High - GPs can discuss AI tools, practice innovation
- **Status**: ✅ Included in final post

**2. Clinical decision support increases primary care clinician confidence in managing cognitive impairment care but not confidence to diagnose**
- **Source**: BMC Primary Care
- **Type**: [International] Research
- **URL**: https://bmcprimcare.biomedcentral.com/articles/10.1186/s12875-025-03053-z
- **Summary**: Study in 38 primary care clinics shows CDS tools increase GP confidence in managing cognitive impairment care (though not diagnosis confidence). 32% remembered the tool, 65% of those used it, and 97% of users said it helped provide better CI care.
- **GP Relevance**: HIGH - Primary care focus, clinical decision support, common GP issue
- **Engagement Potential**: High - GPs can discuss CDS tools, cognitive screening
- **Status**: ✅ Included in final post

**3. Clinical Decision Support Tool May Improve ACEI and ARB Use in CKD**
- **Source**: Medscape
- **Type**: [International] Research/Clinical
- **URL**: https://www.medscape.com/viewarticle/clinical-decision-support-tool-may-improve-acei-and-arb-use-2025a1000vje
- **Summary**: CDS tool may improve underutilized ACEI/ARB prescribing in chronic kidney disease. ACEI and ARB use in CKD has been available for over three decades but is still underutilized.
- **GP Relevance**: HIGH - CKD management is routine GP work; ACEI/ARB prescribing is common
- **Engagement Potential**: High - GPs can discuss prescribing patterns, CDS tools, barriers
- **Note**: Medscape may require login
- **Status**: ✅ Included in final post

#### 📋 Articles Evaluated But Not Included

**4. NZ govt rolls out AI scribe tool to doctors, despite concerns**
- **Source**: Reseller News
- **Type**: [NZ] News
- **URL**: https://www.reseller.co.nz/article/4087164/nz-govt-rolls-out-ai-scribe-tool-to-doctors-despite-concerns.html
- **Summary**: The New Zealand government is rolling out an AI scribe tool to 1,000 doctors and frontline staff in emergency departments nationwide. Health Minister Simeon Brown says this initiative aims to reduce time spent on paperwork, enabling clinicians to focus on patient care. The AI scribe securely records consultations and generates draft clinical notes, referrals, and follow-up summaries tailored to NZ's clinical language and systems, meeting privacy standards with strong security protections. Positive results from pilots in Hawke's Bay and Whanganui led to the nationwide rollout. However, some users found the editing and correction of AI-generated notes time-consuming and expressed concerns about note accuracy, completeness, and conciseness. Criticisms include deployment while regulations and guidelines are still evolving, data privacy/security risks—especially with international cloud storage—and Māori data sovereignty concerns. Only 66% of surveyed providers read the software's terms and conditions, and just 59% sought patient consent.
- **Status**: ❌ Excluded - User requested removal (follow-up from last week)
- **Alternative Use**: Available for standalone post on AI scribe rollout & practice policies

**5. AI is beating doctors at empathy – because we've turned doctors into robots**
- **Source**: NZ City
- **Type**: [NZ] Opinion
- **URL**: https://home.nzcity.co.nz/news/article.aspx?id=433630
- **Summary**: Article discusses burnout and system issues
- **Status**: ❌ Excluded - User decided not to include empathy article (less directly actionable)

**6. [Article title not accessible]**
- **Source**: EurekAlert
- **Type**: Unknown
- **URL**: https://www.eurekalert.org/news-releases/1104853
- **Status**: ❌ Excluded - Could not access article (request blocked)

**7. Putting an AI policy in place protects you and your practice**
- **Source**: Pharmacy Today
- **Type**: [NZ] Opinion/Guidance
- **URL**: https://www.pharmacytoday.co.nz/article/opinion/putting-ai-policy-place-protects-you-and-your-practice
- **Summary**: Health IT expert Sanjeewa Samaraweera discusses why implementing an AI policy and reviewing it regularly is essential for healthcare practices.
- **Status**: ⚠️ PAYWALLED - Not suitable for LinkedIn post (readers can't access)
- **Alternative**: Discussion-only post about AI policies (no article link)

**8. Long and short of it: Investors are turning to healthcare as economic clouds gather**
- **Source**: The Post
- **Type**: [NZ] Business/Investment
- **URL**: https://www.thepost.co.nz/business/360884363/long-and-short-it-investors-are-turning-healthcare-economic-clouds-gather
- **Status**: ❌ Excluded - Investment/finance focused, not GP practice-relevant

**9. Exclusive: Accenture NZ's Dr Will Reedy reveals how AI is reshaping healthcare**
- **Source**: IT Brief
- **Type**: [NZ] Vendor Content
- **URL**: https://itbrief.co.nz/story/exclusive-accenture-nz-s-dr-will-reedy-reveals-how-ai-is-reshaping-healthcare
- **Status**: ❌ Excluded - VENDOR-FOCUSED (Accenture consultancy promoting AI services)
- **Risk**: HIGH - Will attract AI tool vendors (exactly what Week 1 showed to avoid)

**10. Founder admits AI transcription startup...**
- **Source**: Yahoo News
- **Type**: Unknown/Vendor
- **URL**: https://nz.news.yahoo.com/founder-admits-ai-transcription-startup-162025214.html
- **Status**: ❌ Excluded - VENDOR-FOCUSED (Startup founder discussing product)
- **Risk**: HIGH - Will attract vendors, not GPs

**11. Preserving our Heritage in the Light of Innovation: Pacific Data Sovereignty and AI**
- **Source**: MedTechIQ
- **Type**: [NZ] Policy/Ethics
- **URL**: https://www.medtechiq.co.nz/news/preserving-our-heritage-in-the-light-of-innovation-pacific-data-sovereignty-and-ai
- **Status**: ⚠️ BORDERLINE - Important topic but not directly GP practice-relevant
- **Decision**: SKIP unless need filler content

**12. Space tech, smart sensors and AI drive new hope for cerebral palsy**
- **Source**: NZ Herald
- **Type**: [NZ] Research/Innovation
- **URL**: https://www.nzherald.co.nz/nz/space-tech-smart-sensors-and-ai-drive-new-hope-for-cerebral-palsy/UOINB5GR6VEOVJ5IJBSYPLQARA/
- **Status**: ⚠️ BORDERLINE - Interesting innovation story but not directly GP practice-relevant
- **Decision**: SKIP unless need 4th-5th item

#### 📄 Official Guidance Documents (For Future Standalone Post)

**13. MCNZ Statement: Using AI in Patient Care**
- **Source**: Medical Council of New Zealand
- **Type**: [NZ] Official Guidance
- **URL**: [MCNZ Statement URL - to be added]
- **Summary**: Medical Council of New Zealand's policy draft sets out expectations for how doctors should approach the use of artificial intelligence in clinical settings. Key themes include robust legislative and policy frameworks, mandatory informed consent when AI tools affect patient care decisions, protections for patient privacy and data security, equitable outcomes for all patient populations (including specific consideration for Māori), and requirements that GPs understand AI limitations to ensure clinical safety.
- **GP Relevance**: HIGH - Regulatory guidance, authoritative source
- **Status**: Reserved for standalone post on official AI guidance

**14. AI Tools in General Practice Guidance**
- **Source**: Pinnacle Practices
- **Type**: [NZ] Official Guidance
- **URL**: [Pinnacle Practices Guidance URL - to be added]
- **Date**: April 2025
- **Summary**: Pinnacle Practices' April 2025 guidance provides practical steps for safely integrating AI into GP workflows. Four principles: Consent (patients explicitly informed, language adapted to local needs), Check (carefully review all AI outputs for accuracy), Privacy (never upload identifiable patient data without consent), and Secure (ensure all platforms comply with data protection standards). Includes detailed recommendations for adjusting patient forms and internal protocols.
- **GP Relevance**: HIGH - Practical and actionable guidance from major PHO
- **Status**: Reserved for standalone post on official AI guidance

**15. Generative AI and Large Language Models Guidance**
- **Source**: Te Whatu Ora / NAIAEAG
- **Type**: [NZ] Official Guidance
- **URL**: [Te Whatu Ora / NAIAEAG Guidance URL - to be added]
- **Summary**: National guidance from Te Whatu Ora, supported by its AI advisory group (NAIAEAG), cautions that generative AI and large language models (LLMs) should not be used for clinical decision-making or management of sensitive health data without explicit approval. Recommendations include registering applications and consulting NAIAEAG prior to implementation, ensuring rigorous validation for privacy and safety before use. Specific restrictions for clinical use, emphasizing regulatory compliance and organisational review.
- **GP Relevance**: HIGH - National health authority, critical restrictions on clinical use
- **Status**: Reserved for standalone post on official AI guidance

#### 📌 Planned Standalone Posts

**Standalone Post A: AI Scribe Rollout & Practice Policies**
- **Articles**: Item 4 (Reseller News - AI scribe rollout)
- **Theme**: Breaking news / Policy discussion
- **Status**: Draft created, ready to post on different day from weekly roundup

**Standalone Post B: Official AI Guidance for NZ GPs**
- **Articles**: Items 13, 14, 15 (MCNZ, Pinnacle, Te Whatu Ora guidance)
- **Theme**: Policy summary / Official guidance
- **Status**: Draft created, URLs to be added

**Standalone Post C: AI Policy Discussion (Alternative to Paywalled Article)**
- **Articles**: None (discussion-only post inspired by Item 7)
- **Theme**: Practice governance discussion
- **Status**: Draft created as alternative to paywalled content

**Post Theme**: Specialist AI Tools Showing Promise  
**Hook**: "Are specialist AI tools the better path for GPs? Evidence is emerging."

**Hashtags Used**: #PrimaryCareNZ #GeneralPractice #AIinHealth #ClinicalDecisionSupport #DigitalHealthNZ #GPsNZ #HealthTechNZ #CKD #CognitiveImpairment #SpecialistAI #InnovationNZ

**Metrics**: [To be added after posting]

---

*Log updated: 2025-11-27 - All Week 2 articles captured in detail*
