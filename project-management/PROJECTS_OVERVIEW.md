...
- **2026-01-07**: Medtech Integration - 🔄 PHASE 1C IN PROGRESS: Real Commit path implemented. New Lightsail BFF endpoint `POST /api/medtech/session/commit` creates FHIR Media in ALEX using base64 `content.data` with mandatory `identifier`. Vercel commit route now proxies to BFF only and presigns mobile `s3Key` to `downloadUrl`. Widget commit now sends commit-ready sources (desktop base64, mobile s3Key). Next: deploy BFF and run UAT tests (desktop + mobile commit).
