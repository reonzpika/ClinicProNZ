...
- **2026-01-08**: Medtech Integration - Customer-side launch prep started for end-of-Jan Medtech Images launch. Competitive intel gathering initiated (QuickShot by Intellimed; pricing requested). Medtech commercial terms requested. NZ GP Facebook outreach posted to learn real-world clinical photo workflows and pain points (including file size/resizing).
- **2026-01-08**: Medtech Integration - Phase 1D READ investigation: confirmed client-credentials token contains expected app roles (including `patient.task.read`, `patient.media.read`, `patient.communication.*.read`), but direct ALEX UAT calls still return `403` for FHIR search on non-Patient resources (Task, Communication, Media, DocumentReference, DiagnosticReport, MedicationRequest). Emailed Medtech support with the setup and evidence to clarify required search permissions/policy.
- **2026-01-07**: Medtech Integration - ✅ PHASE 1C COMPLETE (deployed, end-to-end): Commit creates FHIR `Media` in ALEX UAT via Lightsail BFF (static IP). Vercel commit route proxies to BFF only; mobile `s3Key` presigned to `downloadUrl`; desktop sends `base64Data`. Evidence: `[Medtech Commit] Commit complete { duration: 3330, successCount: 2, errorCount: 0 }`; BFF smoke test returned `status: committed` with example `mediaId: a4fcaddd4b8f24f377eca1c6b889ad76`.
...
- **Status**: Active — Phase 1C complete (commit to ALEX API working end-to-end). Next: confirm in Medtech UI, harden error handling, decide widget launch mechanism.
