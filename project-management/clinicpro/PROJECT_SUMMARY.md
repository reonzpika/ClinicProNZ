...
- Pages (`app/`):
  ...
  - Clinical: `(clinical)/ai-scribe/consultation`, `(clinical)/ai-scribe/templates`, `(clinical)/ai-scribe/image`, `(clinical)/image` (landing), `(clinical)/image/app` (desktop photo tool), `(clinical)/image/mobile` (mobile capture), `(clinical)/image/upgrade` (pricing), `chat`, `search`, `differential-diagnosis`, `(clinical)/acc/*`
...
- APIs (`app/api/`): grouped under `(clinical)`, `(user)`, `(business)`, `(admin)`, `(integration)`, `(marketing)`, plus `current-session`, `maintenance`, `search`, `image/*` (photo tool freemium).
