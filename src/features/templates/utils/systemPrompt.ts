...
import { buildAiAnalysisExamples, buildAiAnalysisInstructions } from './instructionMapping';
...
export function generateSystemPrompt(
  ...
  let aiExamples: { assessmentExample: string; planExample: string } | undefined;
  ...
  if (aiAnalysisComponents && Object.values(aiAnalysisComponents).some(enabled => enabled)) {
    ...
    aiExamples = buildAiAnalysisExamples(aiAnalysisComponents, aiAnalysisLevel || 'medium');
  }

  return generateBasePrompt(aiAnalysisInstructions, aiExamples);
}
