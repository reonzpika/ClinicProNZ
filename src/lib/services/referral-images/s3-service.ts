/**
 * S3 Image Storage Service for GP Referral Images
 *
 * Manages temporary image storage in S3 with presigned URLs
 *
 * Bucket: clinicpro-referral-images
 * Region: ap-southeast-2 (Sydney)
 * Lifecycle: 24-hour retention (auto-cleanup)
 *
 * S3 Key Format: referral-images/{userId}/{timestamp}_{imageId}.jpg
 */
...
export class ReferralImagesS3Service {
  ...
  constructor() {
    ...
    this.bucketName = process.env.REFERRAL_IMAGES_S3_BUCKET || 'clinicpro-referral-images';
  ...
  /**
   * Generate S3 key for referral image
   */
  private generateS3Key(userId: string, imageId: string): string {
    const timestamp = Date.now();
    return `referral-images/${userId}/${timestamp}_${imageId}.jpg`;
  }
  ...
  async generatePresignedUploadUrl(
    ...
    const s3Key = this.generateS3Key(userId, imageId);
    ...
    const uploadUrl = await retry(
      ...
      },
      {
        ...
        logger: (msg) => {
          console.warn('[ReferralImages-S3] Retry:', msg);
        },
      },
    );
  ...
  /**
   * Generate presigned URL for download with Content-Disposition header for custom filename
   */
  async generatePresignedDownloadUrlWithFilename(
    s3Key: string,
    filename: string,
    ...
    await Promise.all(
      s3Keys.map(async (s3Key) => {
        try {
          ...
          console.error('[ReferralImages-S3] Failed to generate download URL:', s3Key, error);
...
// Singleton instance
export const referralImagesS3Service = new ReferralImagesS3Service();
