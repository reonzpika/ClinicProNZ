...
export class DeepgramService {
    ...
    try {
      ...
      this.connection = deepgram.listen.live({
        ...
        keywords: [
          ...
          'medication', 'prescription', 'history', 'examination',
          // New Zealand specific terms
          'whānau', 'GP', 'ACC', 'DHB', 'PHO',
          'kaimahi', 'tāngata', 'hauora', 'rongoā'
        ],
  ...
  private determineErrorCode(error: any): TranscriptionError['code'] {
    ...
    if (error.message?.includes('audio')) return 'AUDIO_ERROR';
    if (error.message?.includes('redact')) return 'REDACTION_ERROR';
    return 'API_ERROR';
  }
